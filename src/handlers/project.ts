...
// L1-23 (captain 14:52): read-only child counts for cards, one correlated aggregate per row — no N+1.
  const { results } = await ctx.db.prepare('SELECT p.*, g.role, (SELECT COUNT(*) FROM assessment a2 WHERE a2.project_id = p.id) AS assessment_count, (SELECT COUNT(*) FROM response r JOIN assessment_survey s ON s.id = r.assessment_survey_id JOIN assessment a2 ON a2.id = s.assessment_id WHERE a2.project_id = p.id) AS response_count FROM project p JOIN "grant" g ON g.scope_type = ? AND g.scope_id = p.id WHERE g.principal_id = ? ORDER BY p.created_at').bind("project", actor).all<ProjectRow & {role:Role; assessment_count:number; response_count:number}>();
