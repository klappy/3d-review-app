...
export const list: Handler = async (ctx) => {
  const actor = requireUser(ctx);
  const { results } = await ctx.db.prepare('SELECT p.*, g.role FROM project p JOIN "grant" g ON g.scope_type = ? AND g.scope_id = p.id WHERE g.principal_id = ? ORDER BY p.created_at').bind("project", actor).all<ProjectRow & {role:Role}>();
  return { result: { projects: results.map(p => view(p, p.role)) } };
};
