...
// builder: renders only the rows and footer the caller passes; escapes every cell and the footer (heading is trusted markup, as before).
...
export function evidenceTable(rows, open = false, esc = esc0, { footer = '', heading = 'What supports this view?' } = {}) {
  ...
  const table = `<div class="v3-evidence" data-v3-evidence${open ? '' : ' hidden'}><h3>${heading}</h3><table class="table"><thead><tr><th>Evidence</th><th>What we can say</th><th>Limit</th></tr></thead><tbody>${body}</tbody></table>${foot}</div>`;
  return { btn, table };
}
