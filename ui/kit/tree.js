...
export function mountShell(root, initialModel, initialCallbacks = {}) {
  ...
  function paint(focusKey, preserveContent = true) {
    ...
    const pointerdown = e => { if(current() && composing && !search.closest('.tree-search').contains(e.target) && e.target.closest?.('a[href],button,input,select,textarea,[tabindex]:not([tabindex="-1"])')) deferred = true; };
