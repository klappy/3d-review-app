...
export function chrome(model) {
  ...
  return '<header class="top"><a class="brand" href="#" data-navigate="#"><span>3D</span>Review</a><nav class="crumbs" aria-label="Breadcrumb">'+ancestors.map(x=>link(x,x.href===model.currentHref)).join('<span class="sep" aria-hidden="true">›</span>')+badge(model.role)+'</nav><div class="right">'+(model.sample?'<span class="badge demo">SAMPLE DATA · SIMULATED</span>':'')+'<span class="me">'+esc(model.identityLabel)+' · '+esc(model.role)+'</span><span class="header-host" data-header-host></span></div></header>';
}
