...
export const heading=m=>`<div><p class="eyebrow">${esc(m.eyebrow)}</p><h1>${esc(m.title)}</h1>${m.help?`<p class="muted">${esc(m.help)}</p>`:''}</div>`;
...
export function mountCoordinatorView(root,model,onIntent){return mountView(root,model,onIntent,screens,ids,(m,{actions,button})=>{
 ...
 html+=`<div class="${m.screen==='projects'?'three':'grid'}">${cards(m.items)}</div>`;
