...
export function cards(items=[],itemControls=()=>''){return items.map((x,index)=>`<article class="glass panel" style="min-width:0;overflow-wrap:anywhere"><div class="row"><h3>${esc(x.title||x.label)}</h3>${x.role?`<span class="badge">${esc(x.role)}</span>`:''}</div>${x.description?`<p class="muted">${esc(x.description)}</p>`:''}${(x.facts||[]).map(f=>`<p><span class="muted">${esc(f.label)}</span> ${esc(f.value)}</p>`).join('')}${typeof x.responses==='number'?`<p>Responses: ${esc(x.responses)}${typeof x.denominator==='number'?` / ${esc(x.denominator)}`:''}</p>`:''}${(x.children||[]).map(c=>`<div class="nav">${esc(c.label)}<small>${esc(c.detail)}</small></div>`).join('')}${itemControls(x,index)}</article>`).join('');}
export function mountView(root,initial,onIntent,screenSet,actionSet,body){
 let model,callback,generation=0,dead=false,dispatched=new Set(),bindings=[];
 const allowed=a=>a&&actionSet.has(a.id)&&a.allowed===true;
 const blocked=(a,key=a.id)=>a.busy===true||dispatched.has(key)||(['pending','uncertain'].includes(model.interaction?.outcome)&&model.interaction.actionId===a.id)||(a.id==='report-open'&&model.report?.eligibility!=='eligible');
 ...
 const button=(a,values={},scope=null)=>{
  ...
  return `<button type="button" class="${a.primary?'primary':''}" data-intent="${esc(a.id)}" data-binding="${index}" data-values="${esc(JSON.stringify(scope?{}:values))}"${values.view&&values.view===model.view?' aria-current="page"':''}${blocked(a,key)?' disabled':''}>${esc(a.label)}</button>`;
 };
 const itemControls=(item,index)=>{
  if(!item.context||typeof item.context!=='object'||Array.isArray(item.context))return '';
  return `<div class="row">${(item.actions||[]).filter(allowed).map(a=>button(a,a.values&&typeof a.values==='object'&&!Array.isArray(a.values)?a.values:{},{index,context:item.context})).join('')}</div>`;
 };
 function render(){
  generation++;dispatched.clear();bindings=[];
 ...
 function click(e){
  ...
  if(!b||!root.contains(b)||b.closest('[data-slot]')||dead||model.status!=='ready')return;
  const binding=bindings[Number(b.dataset.binding)];
  if(!binding)return;
  const {action:a,key,context,item}=binding;
  if(!allowed(a)||blocked(a,key))return;
  const snapshot=generation,values=structuredClone(binding.values);
  if(a.id!=='browse-view'&&!['navigation','tour-next','tour-back','start-signin'].includes(a.id)){
   if(!item)for(const f of root.querySelectorAll('[data-kit-field][name]')){if(f.closest('[data-slot]'))continue;values[f.name]=f.type==='checkbox'?f.checked:f.value;}
   dispatched.add(key);
   for(const x of root.querySelectorAll('[data-binding]'))if(!x.closest('[data-slot]')&&bindings[Number(x.dataset.binding)]?.key===key)x.disabled=true;
  }
  if(snapshot===generation)callback?.({action:a.id,context:structuredClone(context),values});
 }
...
export function mountCoordinatorView(root,model,onIntent){return mountView(root,model,onIntent,screens,ids,(m,{actions,button,itemControls})=>{
 ...
 if(m.screen==='assessment'&&m.view==='people')html+=`<section class="glass panel" style="overflow:auto"><h3>People</h3><table class="table"><thead><tr><th>Name</th><th>Role</th><th>Access</th><th>Actions</th></tr></thead><tbody>${(m.items||[]).map((x,index)=>`<tr><td>${esc(x.title||x.label)}</td><td style="white-space:nowrap">${esc(x.role)}</td><td>${esc(x.description)}</td><td>${itemControls(x,index)}</td></tr>`).join('')}</tbody></table></section>`;
 else if(m.screen==='project')html+=`<section class="glass panel" style="overflow:auto"><h2>Assessments</h2><table class="table"><thead><tr><th>Assessment</th><th>Context</th><th>Details</th><th>Actions</th></tr></thead><tbody>${(m.items||[]).map((x,index)=>`<tr><td>${esc(x.title||x.label)}</td><td>${esc(x.description)}</td><td>${(x.facts||[]).map(f=>`${esc(f.label)}: ${esc(f.value)}`).join(' · ')}</td><td>${itemControls(x,index)}</td></tr>`).join('')}</tbody></table></section>`;
 else html+=`<div class="${m.screen==='projects'||(m.screen==='assessment'&&m.view==='collect')?'three':'grid'}">${cards(m.items,itemControls)}</div>`;
