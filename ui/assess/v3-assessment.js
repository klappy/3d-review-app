...
export function v3EvidenceRows(results, lenses, groups = {}, scores = null) {
  ...
  return lenses.map(lens => {
    ...
    if (sc) { const n = g.loaded ? g.responses : null, w = v3ScoreBand(sc.score, n);
      return [lens, `${w} (provisional) · score ${Math.round(sc.score)} · ${counted}`, w === 'More input needed' ? `Fewer than ${V3_BAND_CUTOFFS.minResponses} responses` : 'Provisional cut-offs; people who did not answer may see it differently']; }
