...
export const feedback = {
  ...
  render(ctx, model) {
    ...
    return `<section class="panel narrow"><h1>Share app feedback</h1><p>Report a problem or suggest an improvement to 3D Review.</p><p class="small muted">All fields are optional. Your feedback is linked to your signed-in account and is available to authorized support. Please do not include survey answers, other people's personal details, passwords or access codes. ${ctx.feedbackExperience ? 'Only the displayed page category and loaded UI release are attached; no page contents or answers.' : 'Nothing from the page is attached automatically.'}</p><form id="app-feedback"><label class="field">What happened, or what would you improve?<textarea name="note" rows="5" aria-describedby="feedback-note-hint"></textarea></label><p id="feedback-note-hint" class="small muted">For a problem, tell us what happened and what you expected. For a suggestion, describe what would help. Your draft stays here until you leave or reload.</p><details><summary>Rate your experience (optional)</summary><label class="field">Was the app helpful?<select name="helpful"><option value="">Not answered</option><option value="true">Yes</option><option value="false">No</option></select></label>${scores.map(([key, label, low, high]) => `<label class="field">${label}<select name="${key}"><option value="">Not answered</option>${[1,2,3,4,5].map(n => `<option value="${n}">${n}${n === 1 ? ' — ' + low : n === 5 ? ' — ' + high : ''}</option>`).join('')}</select></label>`).join('')}<label class="field">How did the experience change as you went?<input name="sentiment_journey" maxlength="128" placeholder="For example: confused → clear"></label><p class="small muted">Optional short description, up to 128 characters.</p></details><div class="actions"><button type="submit" class="primary">Send feedback</button>${ctx.feedbackModal ? '<button type="button" data-feedback-close>Back to your work</button>' : '<a href="#">Back to home</a>'}</div></form><p id="feedback-status" role="status" aria-live="polite"></p><div id="feedback-receipt"></div></section>`;
  },
  bind(ctx, root, model) {
    ...
    form.onsubmit = async event => {
      ...
      try {
        const result = await ctx.api('/v2/feedback', { method: 'POST', body: { ...body, require_authenticated: true, ...(ctx.feedbackExperience ? { experience: ctx.feedbackExperience } : {}) } });
        if (!current()) return;
