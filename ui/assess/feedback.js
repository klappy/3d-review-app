...
export const feedback = {
  ...
  render(ctx, model) {
    if (!model.allowed) return '<section class="panel narrow"><h1>App feedback</h1><p>Sign in to share feedback about using 3D Review. The sample tour does not send feedback.</p><a class="button" href="/v2/auth/email">Sign in</a> <a href="/">Back to home</a></section>';
    ...
    form.onsubmit = async event => {
      ...
      try {
        ...
        if (['401','NOT_AUTHENTICATED'].includes(code)) {
          ...
          const link = root.ownerDocument.createElement('a'); link.href = '/v2/auth/email'; link.textContent = ' Sign in again'; status.append(link);
