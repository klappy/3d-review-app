...
import { v3CountLine, v3BandsMarkup, v3EvidenceRows, v3EvidenceMarkup, v3StageWord, V3_FLAGS, v3css, v3ReviewGateMarkup, v3SetStage, V3_SET_STAGE, V3_NEXT, v3NextFinishes } from './v3-assessment.js'; // v3 lane 3 (rulings a/b/c) // relative: resolves at /report-view.js in the browser and under node --test
  ...
  bind(ctx, root, m) {
    ...
    form.onsubmit = async e => {
      ...
      try {
        ...
        if (V3_FLAGS.nextStepPage && ctx.current?.assessment) Object.assign(ctx.current.assessment, { notes_reflection: m.notes_reflection, notes_next_steps: m.notes_next_steps }); // Bugbot 4095257240: a failed refetch remounts the saved notes
        const left = () => (ctx.isCurrent && !ctx.isCurrent()) || root.isConnected === false; // Bugbot 4095257215: never repaint a route the user moved to
        ...
        if (left()) { if (ctx.state?.dirty instanceof Map) ctx.state.dirty.set(m.aid, 'write'); return; }
