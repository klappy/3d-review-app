...
export function route(hash) {
  ...
  return { kind: 'assessment', id: parts[1], phase: PHASES.includes(parts[2]) ? parts[2] : null };
}

async function assessmentsFor(pid) {
  if (!state.assessmentsByProject.has(pid)) { const r = await api(`/v2/projects/${encodeURIComponent(pid)}/assessments`); state.assessmentsByProject.set(pid, r.assessments || []); }
  return state.assessmentsByProject.get(pid);
}
function context(current) {
  const projects = state.projects.map(p => {
    const list = state.assessmentsByProject.get(p.id);
  ...
  return `<aside class="context-panel"><p class="eyebrow">Projects</p><nav aria-label="Project and assessment navigation">${projects}</nav><div class="line links"><a href="#">All projects</a></div></aside><section class="assessment-body">`;
}
function tabs(a, current) { return `<nav class="tabs" aria-label="Assessment phases">${PHASES.map(p => `<a href="#assessment/${encodeURIComponent(a.id)}/${p}" ${p === current ? 'aria-current="page"' : ''}>${title(p)}${p === a.stage ? ' <span class="small">· current</span>' : ''}</a>`).join('')}</nav>`; }
function lensRows(current) {
  ...
  return groupByLens({ surveys: current.surveys, templates: state.templates || [] }).map(g => {
    const included = g.included.map(s => `<div class="survey"><span class="dot ${DOTS[g.lens] || ''}"></span><div><h3>${esc(s.template_name)}</h3><p class="small muted">v${esc(s.template_version)} · ${esc(s.collection_status)}</p></div>${mayEdit ? `<button class="quiet" data-remove="${esc(s.id)}" ${state.busy ? 'disabled' : ''}>Remove</button>` : ''}</div>`).join('');
...
function screen(current, phase) {
  const a = current.assessment, project = state.projects.find(p => p.id === a.project_id);
  const surveySet = `<aside class="panel"><p class="eyebrow">Survey set</p><h2>Three lenses</h2><p class="muted">Within each lens, choose which surveys this assessment includes.${state.templates ? '' : ' Template catalogue not loaded.'}</p>${lensRows(current)}<p class="status" role="${state.message ? 'alert' : 'status'}" aria-live="polite">${esc(state.message)}</p></aside>`;
  const left = `<section class="panel"><p class="eyebrow">${title(phase)}</p><h2>${{ prepare: 'Prepare this assessment', collect: 'Collect perspectives', understand: 'Bring the perspectives together', improve: 'What comes next?' }[phase]}</h2><p class="muted">Current stage: <strong>${title(a.stage)}</strong>. Stage changes, invitations, links, reports and notes are not on this screen yet; the current workspace still has them.</p></section>`;
  return `<div class="title"><div><p class="eyebrow">Assessment</p><h1>${esc(a.name)}</h1><p class="muted" style="margin:0">${esc(project?.name || a.project_id)} · your role: ${esc(a.role)}</p></div><span class="badge">${stageLabel(a.stage)}</span></div>${tabs(a, phase)}<div class="grid">${left}${surveySet}</div>`;
}
...
function bind(current) {
  ...
  app.querySelectorAll('[data-include]').forEach(b => b.onclick = () => act('Including survey…', () => api(`/v2/assessments/${encodeURIComponent(aid)}/surveys`, { method: 'POST', body: { template_id: b.dataset.include, version: Number(b.dataset.version) } })));
