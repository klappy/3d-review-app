...
const workspace = {
  ...
  render(ctx, model) {
    ...
    const cards = model.projects.map(p => ctx.cards.projectCard(p));
    const rows = edit && model.projects.length ? `<div class="links" style="margin-top:18px">${model.projects.map(p => `<div class="row"><span>${ctx.esc(p.name)}</span><button type="button" class="quiet" data-remove="${ctx.esc(p.id)}">Remove from workspace</button></div>`).join('')}</div>` : '';
