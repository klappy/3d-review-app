...
// U28 (Bincy B32, captain ruling 2026-09-25 "she should be able to sign up herself"): a new email is accepted and its account is
// created on sign-in, so both sign-in entry points (public "/" choice and #signin) say so — one shared line, under the Sign in action.
export const SIGNUP_NOTE = 'New here? Signing in with your email creates your account.';
// Its own full-width line after the Sign in action: flex-basis for the flex .actions row (product "/" and #signin), grid-column for the
// grid public-choices row (legacy public-choices.css). Bugbot on #301: a last-column-only placement lands as a fifth flex item on "/".
const signupNote = ctx => `<small class="small muted signup-note" data-signup-note style="display:block;flex-basis:100%;grid-column:1/-1;margin:0">${ctx.esc(SIGNUP_NOTE)}</small>`;
...
const entry = {
  ...
  render(ctx, model) {
    ...
    switch (model.mode) {
      ...
      case 'signin': return signinView(ctx, model, signedIn); // U28: the new-account line is for signed-out visitors only
