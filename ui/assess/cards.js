...
export function card({ eyebrow, title, href, meta = [], badge = '', note = '', archived = false, id = '' }) {
  const tag = href ? 'a' : 'div';
  const attrs = `class="panel entity-card${archived ? ' archived' : ''}"${href ? ` href="${esc(href)}"` : ''}${id ? ` data-id="${esc(id)}"` : ''}`;
  return `<${tag} ${attrs}>${eyebrow ? `<p class="eyebrow">${esc(eyebrow)}</p>` : ''}<h2>${esc(title)}</h2>${meta.filter(Boolean).map(m => `<p class="muted small">${esc(m)}</p>`).join('')}${badge ? `<span class="badge">${esc(badge)}</span>` : ''}${note ? `<p class="small muted">${esc(note)}</p>` : ''}</${tag}>`;
}
...
export const workspaceCard = w => card({ eyebrow: 'Workspace', title: w.name, href: routes.workspace(w.id), meta: [w.role ? `Your role: ${w.role}` : ''], badge: w.archived_at ? 'Archived' : '', archived: !!w.archived_at, id: w.id });
export const projectCard = p => card({ eyebrow: 'Project', title: p.name, href: routes.project(p.id), meta: [p.role ? `Your role: ${p.role}` : ''], badge: p.archived_at ? 'Archived' : '', archived: !!p.archived_at, id: p.id });
export const assessmentCard = a => card({ eyebrow: 'Assessment', title: a.name, href: routes.assessment(a.id), meta: [a.language_name || a.language_id ? `Language: ${a.language_name || a.language_id}` : ''], badge: a.archived_at ? 'Archived' : stageLabel(a.stage), archived: !!a.archived_at, id: a.id });
