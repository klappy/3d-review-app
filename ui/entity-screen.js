...
// derived from actual selection state (the existing #projects/#assessments/#granted-assessments selects and the W manager's selected
...
export function mountEntityScreen(doc, win, { isStaff, selectedWorkspace, backToWorkspaces }) {
  ...
  const granted = doc.getElementById('granted-assessments');
  ...
  function level() { return entityLevel({ staff: isStaff(), workspace: selectedWorkspace(), project: projects.value, assessment: assessments.value || granted?.value, survey: surveys?.value }); }
  ...
  function render() {
    ...
    if (back) {
      const label = l === 'survey' ? 'Back to assessment' : l === 'assessment' ? (projects.value ? 'Back to project' : 'Back to workspaces') : l === 'project' ? 'Back to workspaces' : l === 'workspace' ? 'Back to workspaces' : '';
  ...
  back?.addEventListener?.('click', () => {
    ...
    else if (l === 'assessment') { if (assessments.value) select(assessments, ''); else select(granted, ''); }
  ...
  if (typeof win.MutationObserver === 'function') { // option refreshes, identity text and W renders arrive without events; the selects' change events still drive render without an observer
    ...
    for (const id of ['projects', 'assessments', 'surveys', 'granted-assessments', 'identity', 'workspace-manager-root']) { const n = doc.getElementById(id); if (n) observer.observe(n, { childList: true, subtree: true, attributes: true, characterData: true }); }
    { const n = doc.getElementById('facilitator'); if (n) observer.observe(n, { attributes: true, attributeFilter: ['hidden'] }); }
  }
  projects.addEventListener('change', render); assessments.addEventListener('change', render); granted?.addEventListener('change', render); surveys?.addEventListener('change', render);
