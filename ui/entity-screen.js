...
// This controller owns exactly one body attribute, data-entity-level ∈ workspaces | workspace | project | assessment,
...
export function entityLevel({ staff, workspace, project, assessment }) {
...
export function mountEntityScreen(doc, win, { isStaff, selectedWorkspace, backToWorkspaces }) {
  const body = doc.body, projects = doc.getElementById('projects'), assessments = doc.getElementById('assessments');
  ...
  function level() { return entityLevel({ staff: isStaff(), workspace: selectedWorkspace(), project: projects.value, assessment: assessments.value }); }
  ...
  function render() {
    ...
    if (back) {
      const label = l === 'assessment' ? 'Back to project' : l === 'project' ? 'Back to workspaces' : l === 'workspace' ? 'Back to workspaces' : '';
  ...
  back?.addEventListener?.('click', () => {
    const l = level();
    if (l === 'assessment') select(assessments, '');
  ...
  if (typeof win.MutationObserver === 'function') { // option refreshes, identity text and W renders arrive without events; the selects' change events still drive render without an observer
    ...
    for (const id of ['projects', 'assessments', 'identity', 'workspace-manager-root']) { const n = doc.getElementById(id); if (n) observer.observe(n, { childList: true, subtree: true, attributes: true, characterData: true }); }
    { const n = doc.getElementById('facilitator'); if (n) observer.observe(n, { attributes: true, attributeFilter: ['hidden'] }); }
  }
  projects.addEventListener('change', render); assessments.addEventListener('change', render);
