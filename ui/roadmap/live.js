...
export function createLiveFeed({fetcher=globalThis.fetch,EventSourceClass=globalThis.EventSource,onChange,clock=Date.now,setTimer=setTimeout,clearTimer=clearTimeout}){
 let disposed=false,paused=false,source=null,timer=null,busy=false,pending=false,epoch=0,cursor=0,generation=null,inconsistencies=0;
 ...
 function connect(){
  ...
  source.onopen=()=>{state.connection='connected';emit();};
  const message=(e,reset=false)=>{let data;try{data=JSON.parse(e.data);}catch{return;}if(!Number.isSafeInteger(data.cursor)||!Number.isSafeInteger(data.generation))return;
  ...
  source.addEventListener('unavailable',()=>{state.connection='disconnected · polling fallback';emit();schedule();});
  source.onerror=()=>{state.connection='disconnected · polling fallback';emit();schedule();};
 }
