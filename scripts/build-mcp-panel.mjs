...
const actionCard = readFileSync(new URL('../ui/mcp/action-card.js', import.meta.url), 'utf8').replace(/^export\s+(const|function)\s/gm, '$1 ');
const actionStyle = readFileSync(new URL('../ui/mcp/action-card.css', import.meta.url), 'utf8');
const src = readFileSync(new URL('../ui/mcp/panel-src.html', import.meta.url), 'utf8');
const themeTokens = readFileSync(new URL('../ui/design-system/tokens.css', import.meta.url), 'utf8');
const themeStyle = readFileSync(new URL('../ui/mcp/theme.css', import.meta.url), 'utf8');
...
const html = `<!doctype html><html lang="en" data-theme-pending><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>3D Review</title><style>${style}${extra}${themeTokens}${themeStyle}${actionStyle}</style></head><body class="mcp-compact">${src.replace('/*__APP_SDK_BUNDLE__*/', () => sdkGlobal).replace('/*__CARDS__*/', () => cards).replace('/*__ACTION_CARD__*/', () => actionCard)}</body></html>`;
...
const dest = new URL('../src/mcp-panel.html', import.meta.url);
if (process.argv.includes('--check')) {
  const current = readFileSync(dest, 'utf8');
  if (current !== html) { console.error('src/mcp-panel.html is stale; run: node scripts/build-mcp-panel.mjs'); process.exit(1); }
  console.log('src/mcp-panel.html matches source', html.length, 'bytes');
} else {
