...
const html = `<!doctype html><html lang="en" data-theme-pending><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>3D Review</title><style>${style}${extra}${themeTokens}${themeStyle}</style></head><body>${src.replace('/*__APP_SDK_BUNDLE__*/', () => sdkGlobal).replace('/*__CARDS__*/', () => cards)}</body></html>`;
