...
- Duplicate-redemption revocation is best-effort under true concurrency: the loser's `revokeGrant` and the winner's grant write race in KV, so the winner's tokens may survive (deny is the MUST and always holds; revoke is the SHOULD). Sequential reuse is refused by the provider itself, which revokes deterministically.
