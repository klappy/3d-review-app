...
- **Well-formed unknown tokens cost 2 indexed D1 SELECTs** (`session`, then `participant_session`) before any limiter, because resolution precedes metering; the caller then collapses to anonymous and the anonymous budget bounds the *next* request from that address. **Residual R1 (numbered, PROPOSED pending independent reviewer acceptance):** a flood of well-formed random tokens from one address is bounded to ≤ 60 requests / min on HTTP → ≤ 120 reads / min / address (30 → 60 on MCP), 0 writes. A pre-resolution credential meter was considered and rejected as a second binding for the same budget; revisit only on a field receipt showing abuse.
