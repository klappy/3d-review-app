...
- PROPOSED · ACCEPTED by independent review 5708128737 — A well-formed but unknown provider-shaped bearer (`a:b:c`) costs 1 KV read and then, through the external-token fallthrough, the two first-party `session`/`participant` SELECTs before its 401 — bounded by `RL_MCP_CEILING` (600/min per address → ≤ 1 200 D1 reads/min per address). Not a bypass; a cost.
