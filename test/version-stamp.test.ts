...
describe("manifest records pin the cookbook byte copies (git blob id + sha256)", () => {
  const expectedPaths = ["planning/2026-09-16-parity-build/releases/0.7.0.md", "planning/2026-09-16-parity-build/releases/releases.json"];
  ...
  it("manifest.changelog.sections deep-equal the pinned releases.json 0.7.0 sections; markdown bullets equal the json verbatim", () => {
    const entry = releases.versions.find((v: any) => v.version === "0.7.0");
    ...
    const md = read("release/cookbook/0.7.0.md").toString("utf8");
    ...
    expect(md).toContain("## [0.7.0] — candidate");
  });
});

describe("generated ui/changelog.json", () => {
  it("deep-equals the derived shape: current, newest-first, 0.7.0 candidate with no tag/date, four section keys, no HTML", () => {
    ...
    expect(gen.versions[0].version).toBe("0.7.0");
...
describe("runtime surfaces: health and MCP serverInfo", () => {
  ...
  it("GET /v2/health carries version/build/commit/release_source and keeps contract/source_sha/deps/capabilities(84)/ok", async () => {
    ...
    expect(h.build).toBe(`0.7.0+${APP_COMMIT.slice(0, 7)}`);
  ...
  it("MCP initialize → serverInfo.version === APP_VERSION", async () => {
    ...
    expect(body.result.serverInfo.version).toBe("0.7.0");
  });
});
...
describe("stamp script (subprocess): CI without WORKERS_CI_COMMIT_SHA fails, never fabricates", () => {
  ...
  it("CI=true with a 40-hex WORKERS_CI_COMMIT_SHA → exit 0 and the generated file (in the temp dir) carries that sha and build uuid", () => {
    ...
    expect(gen).toContain(`export const APP_STAMP = "0.7.0+aaaaaaa";`);
    expect(gen).toContain(`export const BUILD_UUID: string | null = "11111111-2222-3333-4444-555555555555";`);
    expect(r.stdout).toContain("0.7.0+aaaaaaa");
