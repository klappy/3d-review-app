...
describe("version authority chain: package == lock == manifest == generated", () => {
  it("package.json, both package-lock.json version fields and the manifest agree with the generated APP_VERSION", () => {
    expect(manifest.version).toBe("0.14.4");
...
describe("manifest records pin the cookbook byte copies (git blob id + sha256)", () => {
  const expectedPaths = ["planning/2026-09-16-parity-build/releases/0.14.4.md", "planning/2026-09-16-parity-build/releases/releases.json"];
  ...
  it("manifest.changelog.sections deep-equal the pinned releases.json 0.14.4 sections; markdown bullets equal the json verbatim", () => {
    const entry = releases.versions.find((v: any) => v.version === "0.14.4");
    ...
    const md = read("release/cookbook/0.14.4.md").toString("utf8");
    ...
    expect(md).toContain("## [0.14.4] — candidate");
  });
});

describe("generated ui/changelog.json", () => {
  it("deep-equals the derived shape: current, newest-first, 0.14.4 candidate with no tag/date, four section keys, no HTML", () => {
    ...
    expect(gen.versions[0].version).toBe("0.14.4");
...
describe("runtime surfaces: health and MCP serverInfo", () => {
  ...
  it("GET /v2/health carries version/build/commit/release_source and keeps contract/source_sha/deps/capabilities(90)/ok", async () => {
    ...
    expect(h.build).toBe(`0.14.4+${APP_COMMIT.slice(0, 7)}`);
  ...
  it("MCP initialize → serverInfo.version === APP_VERSION", async () => {
    ...
    expect(body.result.serverInfo.version).toBe("0.14.4");
  });
});
