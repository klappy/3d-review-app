...
describe("A4 minors (review #15 findings 5–9, 11)", () => {
  ...
  it("(5)(6) non-token shapes and provider-divergent Bearer spellings → 401 with 0 D1 and 0 KV", async () => {
    ...
    // the real provider shape still passes the gate: 1 KV read, then the provider's external-token fallthrough reaches
    // resolvePrincipal, whose token-shape gate (PR #19 B3) answers anonymous WITHOUT a SELECT → 401. Stitched tree
    // (PR #19 + PR #20): 1 KV get, 0 D1 — was 1 KV + 2 SELECTs on PR #20 alone (plan v2 item 3 residual, now closed by the gate).
    const r = await mcpCall(e, `Bearer usr_0123456789abcdef0123:${g16}:${s32}`); expect(r.status).toBe(401);
    expect(kv).toEqual(["get"]); expect(sqls.length).toBe(0);
  }, 60_000);
