...
describe("Lane B: grant / request / support.unlock (D3, no inheritance, existence hidden)", () => {
  it("runs the D3 rules end to end on a fresh D1", async () => {
    ...
    expect(sent.result.delivered).toBe(false); expect((sent.result.delivery as any).reason).toBe("not_configured"); /* this env has no PUBLIC_ORIGIN: no link can be built, so nothing may be sent */ expect(sent.result.status).toBe("pending");
    ...
    expect(again.result.invitation_id).toBe(sent.result.invitation_id); expect((again.result.delivery as any).reason).toBe("duplicate_recent");
    ...
    expect(supportHandlers["cap.support.unlock_participant"]).toBe(unlock_participant);
    await expect(unlock_participant(owner, { code_id: "code_old", reason: "lost" })).rejects.toMatchObject({ code: "NOT_AUTHORIZED_AT_SCOPE" });
    const u = await execute(support, "cap.support.unlock_participant", { code_id: "code_old", reason: "participant lost the sheet" }, { tool: "write" });
