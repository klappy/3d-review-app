...
describe("Lane B: grant / request / support.unlock (D3, no inheritance, existence hidden)", () => {
  it("runs the D3 rules end to end on a fresh D1", async () => {
    ...
    const acc = await accept(stranger, { token });
    expect(acc.result).toMatchObject({ granted: true, role: "member" });
    await expect(accept(stranger, { token })).rejects.toMatchObject({ code: "INVALID_PARAMS" }); // invitation_used
    ...
    const l = await list(owner, scope); const rinaGrant = (l.result.grants as any[]).find((g) => g.principal_id === "usr_x");
