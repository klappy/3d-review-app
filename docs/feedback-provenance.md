...
## In-place feedback context (issue130, candidate0.15.0)

The authenticated application shell opens the existing form in a native dialog without replacing the current route or work DOM. Closing/reopening preserves the draft for that route and identity; departure clears it and suppresses late receipts. Direct feedback links remain compatible. Demo and anonymous modes retain their existing refusal behavior.

The optional shared HTTP/MCP `experience.context` object accepts exactly `page` and `component`. Page is one of workspaces, workspace, projects, project, assessment, survey, permissions, feedback, unknown; component is app_feedback. No arbitrary routes, URLs, identifiers, inputs or answers are accepted in this context. Agents may report the same registered context through the existing feedback capability. The UI captures occurrence time, web/browser surface and its loaded generated client-release module. A later health response is never substituted for that client identity; API identity is omitted unless actually observed. Submission-server provenance remains separately stamped as before. Mixed cached assets cannot be proven coherent merely by this module identity.

Screenshot uploads, element picking, aggregation and community governance remain separate queued work. This slice does not complete all of issue130.
