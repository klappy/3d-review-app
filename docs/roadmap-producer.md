...
Optional reviewed card badges: work_type (bugfix/feature/improvement/docs/research/maintenance), release_impact (patch/minor/major), release_reference (explicit bounded semantic version), breaking (boolean). Each may be null to clear. Never infer these from titles or workflow. A reference is not verification and a badge does not assign a release automatically.
