...
The [canonical additive contract](https://github.com/klappy/3d-review-cookbook/blob/221aac36f587e2fc943296450193adbbc09dc1cb/planning/2026-09-18-live-roadmap/AMENDMENT.md) defines six capabilities through the existing docs/read/write/danger tools. Their HTTP twins and the UI share the same dispatcher/store. There are no UI-only mutations. D1 stores durable public current items/events separately from restricted publisher audit. Snapshot, replay page and cursor share a transaction. Status publication does not require an app deployment.
